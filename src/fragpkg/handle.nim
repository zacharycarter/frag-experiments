...
proc handleCreatePool*(capacity: int32): ptr HandlePool =
...
proc handleDestroyPool*(pool: ptr HandlePool) =
  if pool != nil:
    deallocShared(pool)
...
proc handleGrowPool*(ppool: ptr ptr HandlePool): bool =
  let 
    ...
    newPool = handleCreatePool(newCap)
  ...
  handleDestroyPool(pool)
  ppool[] = newPool

  result = true

proc handleNew*(pool: ptr HandlePool): Handle =
...
template handleNewAndGrow*(pool: untyped): untyped =
  if handleFull(pool):
